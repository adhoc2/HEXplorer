    ...
    QDialog(parent), ui(new Ui::ChooseLabel)
{
    ...
    connect(&timer, SIGNAL(timeout()), this, SLOT(on_lineEdit_textChanged_timer()));
    connect(ui->pushButton, SIGNAL(clicked()), this, SLOT(on_lineEdit_textChanged_timer()));
        ...
        QDialog(parent), ui(new Ui::ChooseLabel)
{
    ...
    connect(&timer, SIGNAL(timeout()), this, SLOT(on_lineEdit_textChanged_timer()));
    connect(ui->pushButton, SIGNAL(clicked()), this, SLOT(on_lineEdit_textChanged_timer()));
    connect(ui->lineEdit_2, SIGNAL(editingFinished()), this, SLOT(on_lineEdit_textChanged_timer()));
    connect(ui->listWidget, SIGNAL(currentItemChanged(QListWidgetItem*,QListWidgetItem*)), this, SLOT(selectedItem(QListWidgetItem*,QListWidgetItem*)));
...
void ChooseLabel::on_lineEdit_textChanged_timer(QString str)
{
    ...
    if (ui->lineEdit->text().startsWith('/') && ui->lineEdit->text().size() >= 2)
    {
        ...
        if (fun)
        {
            ...
            if (tutu.count() == 2)
            {
                ...
                text.replace(".", "\\.");
                text.replace("*", ".*");
                QStringList result = strList.filter(QRegularExpression("^" + text, QRegularExpression::CaseInsensitiveOption));
    ...
    else if (ui->lineEdit->text().size() >= 1)
    {
        ...
        if (module)
        {
            ...
            //QStringList result = list.filter(QRegularExpression("^(" + text + ')', Qt::CaseInsensitive));
            QStringList result = list.filter(re);
            //qDebug() << re;
...
void ChooseLabel::on_remove_all_clicked()
{
    ...
    QString str;
    on_lineEdit_textChanged_timer(str);
}
