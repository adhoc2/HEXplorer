...
ChTextEdit::ChTextEdit(QWidget *parent) : QsciScintilla(parent)
{
    ...
    connect(this, SIGNAL(customContextMenuRequested(QPoint)), this, SLOT(mycontextMenuEvent()));
...
void ChTextEdit::mycontextMenuEvent ( )
{
...
void ChTextEdit::findNextText()
{
 ...
 int ChTextEdit::lineNumberAreaWidth()
 {
     ...
     int space = 3 + (fm.horizontalAdvance(QLatin1Char('9')) * digits) + digits;
