...
void SpreadsheetView::createActions()
{
    ...
    pasteActionObd = new QAction(tr("&Paste"), this);
    pasteActionObd->setIcon(QIcon(":/icones/paste.png"));
    pasteActionObd->setShortcut(tr("Ctrl+V"));
    pasteActionObd->setShortcutContext(Qt::WidgetShortcut);
    ...
    saveAs = new QAction(tr("export all OBD view as..."), this);
    ...
    saveModifsAs = new QAction(tr("export modifications as..."), this);
    saveModifsAs->setIcon(QIcon(":/icones/milky_saveas.png"));
    //resetAllFilters->setShortcut(Qt::CTRL + Qt::Key_F);
    connect(saveModifsAs, SIGNAL(triggered()), this, SLOT(saveObdViewModifsAs()));
    saveModifsAs->setDisabled(false);
        ...
        else if (name.toLower().endsWith("obdmergemodel")  || name.toLower().endsWith("obdsortfilterproxymodel") )
        {
            ...
            if (index.column() > 1)
            {
                ...
                foreach (QString value, uniqueList)
                {
                    ...
                    if (action->isChecked())
                        connect(action, &QAction::triggered, this, [this, action]() { this->filterColumn(action->text(), false); });
                    else
                        connect(action, &QAction::triggered, this, [this, action]() { this->filterColumn(action->text(), true); });
                }
            }
...
void SpreadsheetView::findInObdView()
{
    ...
    if (ok && !searchText.isEmpty())
    {
        ...
        else
        {
            ...
            obdSortFilterProxyModel *proxyModel = (obdSortFilterProxyModel*)model();
            QModelIndexList indexList = proxyModel->match(proxyModel->index(0,col), Qt::DisplayRole, searchText);

            if (!indexList.isEmpty())
                this->setCurrentIndex(indexList.at(0));
...
void SpreadsheetView::saveObdViewAs()
{
    ...
    else if (exportFormat == "dcm" && model()->inherits("obdSortFilterProxyModel"))
    {
        ...
        // display information
        MDImain *win = (MDImain*)this->mdimain;
        win->writeOutput("export OBD view into dcm file performed with success.");
    }

}

void SpreadsheetView::saveObdViewModifsAs()
{
    //choose format
    QString exportFormat = "";
    DialogChooseExportFormat *chooseFormat = new DialogChooseExportFormat(&exportFormat, this);
...
void SpreadsheetView::setFilter_Pattern()
{
    QLineEdit *txt = new QLineEdit();
    QString pattern = "";
    txt->setText(pattern);
    connect(txt, &QLineEdit::textChanged, this, [this, txt]() { this->filterColumn_Name(txt->text()); });
    txt->show();
}
...
void SpreadsheetView::filterColumn_Name(QString text)
{
    ...
    if (name.toLower().endsWith("obdsortfilterproxymodel"))
    {
        ...
        if (text.isEmpty())
        {
            proxyModel->addFilter(1, ".*");
        }
        else
        {
            text.replace("*", ".*");
            proxyModel->addFilter(1, text + ".*");
        }

    }
}

void SpreadsheetView::importLabFileinObdView()
{
    ...
    if (lines.count() != 0)
    {
        ...
        else
        {
            ...
            obdModel->setErrorCodes(strList);
...
void SpreadsheetView::pasteObdView()
{
    //first check if items are selcted to paste
    QModelIndexList indexList = this->selectedIndexes();
    if (indexList.isEmpty())
        return;

    //get data from CLipboard and count the number of rows and cols
    QString str = QApplication::clipboard()->text();
    QStringList listRowsClip = str.split('\n');
    // int numRowsClip = listRowsClip.count();
    // int numColsClip =listRowsClip.at(0).split("\t").count();


    //case1 : only one index selected => copy all rows and cols from this selected index
    if (indexList.count() == 1)
    {
        //map selectecindexes to the source one in case of filter
        obdSortFilterProxyModel *proxyModel = (obdSortFilterProxyModel*)model();
...
void SpreadsheetView::myResize( int b)
{
