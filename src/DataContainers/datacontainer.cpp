...
DataContainer::DataContainer(WorkProject *parentWP, QString module) : Node((Node*)parentWP, parentWP->a2lFile->lex, parentWP->a2lFile->errorList)
{
    parentWp = parentWP;
...
DataContainer::DataContainer(WorkProject *parentWP) : Node((Node*)parentWP)
{
    parentWp = parentWP;
    isSortedBySubset = false;
    //moduleName = module;
...
bool DataContainer::exportDataList2Cdf(QStringList dataList, QString fileName)
{
    ...
    //03-2022 : code modification to replace whitesoace indentation with tabs
    QString xml = doc.toString();
    QStringList listorg;
    QStringList listnew;
    listorg = xml.split("\n");
    foreach (QString str, listorg)
    {
        if (str.length() > 0)
        {
            int i = 0;
            while (str.at(i) == QString(" "))
            {
                str.replace(i,1,'\t');
                if (str.length() - i >= 2)
                    i++;
    ...
    for ( QStringList::Iterator it = listnew.begin(); it != listnew.end(); ++it )
                    ts << *it;
...
bool DataContainer::exportDataList2Dcm(QStringList dataList, QString fileName)
{
    ...
    if (fileName == "")
    {
        ...
        fileName = QFileDialog::getSaveFileName(0,"select DCM files", path,
                                                        "DCM files (*.dcm);;all files (*.*)");


        if (fileName.isEmpty())
            return false;
        else if (!fileName.toLower().endsWith(".dcm"))
            fileName.append(".dcm");
    }

    //set the csvFilePath into settings
    QSettings settings(qApp->organizationName(), qApp->applicationName());
    QString currentDcmPath = QFileInfo(fileName).absolutePath();
    settings.setValue("currentDcmPath", currentDcmPath);
    ...
    //HEADER PART
    out << "* DAMOS-Austauschdatei\r\n"
           "* Erstellt von HEXplorer v" << qApp->applicationVersion() << "\r\n"
           "* Erstellungsdatum: " << QDate::currentDate().toString() <<
    ...
    foreach (QString label, dataList)
    {
        ...
        {
            FUNCTION *subset = (FUNCTION*)fun->getNode(data->getSubset());
            data->writeValue2Dcm(subset, out);
        }
        else
            data->writeValue2Dcm(0, out);
    }
