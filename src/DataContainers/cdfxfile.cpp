...
//         // int column() const
//         // {
//         //     return m_context.column();
//         // }

//     protected:
//         virtual void messageHandler(QtMsgType type, const QMessageLogContext &context, const QString &msg)
//         {
//             Q_UNUSED(type)
//             // Construct a log message with additional context information
//             logMessage = QString("%1:%2 - %3")
//                                      .arg(context.file)
//                                      .arg(context.line)
//                                      .arg(msg);

//             //QSourceLocation location = context.location;
//             // m_messageType = type;
//             // m_message = msg;
//             // m_context = context;
//             // m_sourceLocation = context.file;
//         }
...
bool CdfxFile::validate(QFile *file)
{
    ...
    // parser.setExternalNoNamespaceSchemaLocation(":/files/cdf_v2.0.0.cl.xsd");

    // // Parse the XML document and validate it against the schema
    // parser.parse(file->fileName().toLocal8Bit());
...
void CdfxFile::parseSwInstanceTree(QDomNode &node)
{
    ...
    for (int i = 0; i < childSwInstanceTree.count(); i++)
    {
        ...
        {
            QString category = node.firstChild().nodeValue();
            if (category != "" and category != "VCD" && category != "NO_VCD") //02-03-2022 => added category != "" due to an issue with some exports from Creta
            {
