...
bool SrecFile::parseFile()
{
    ...
    while (!in.atEnd())
    {
        ...
        else if (str.startsWith(QChar(0x001A)))
        {
            // ignore line
...
void SrecFile:: readAllData()
{
    ...
    if (0)
    {
        ...
        if (length > 10000 && omp_get_num_procs() > 1)
        {
            ...
            QFuture<void> t1 = QtConcurrent::run([this, list1, &listData1, nodeChar, nodeAxis]() {
                this->runCreateData(list1, &listData1, nodeChar, nodeAxis);
            });
            //QFuture<void> t2 = QtConcurrent::run(this, &SrecFile::runCreateData, list2, &listData2, nodeChar, nodeAxis);
            QFuture<void> t2 = QtConcurrent::run([this, list1, &listData1, nodeChar, nodeAxis]() {
                this->runCreateData(list1, &listData1, nodeChar, nodeAxis);
            });
            t1.waitForFinished();
            t2.waitForFinished();
    ...
    if (0)
    {
        ...
        if (length > 10000 && omp_get_num_procs() > 1)
        {
            ...
            QFuture<void> t1 = QtConcurrent::run([this, &listData1, &listChar1, &listAxis1]() {
                this->runCreateData2(&listData1, &listChar1, &listAxis1);
            });
            //QFuture<void> t2 = QtConcurrent::run(this, &SrecFile::runCreateData2, &listData2, &listChar2, &listAxis2);
            QFuture<void> t2 = QtConcurrent::run([this, &listData2, &listChar2, &listAxis2]() {
                this->runCreateData2(&listData2, &listChar2, &listAxis2);
            });
            t1.waitForFinished();
            t2.waitForFinished();
    ...
    if (type.endsWith("CHARACTERISTIC"))
    {
        ...
        if (type.toLower() == "value")
        {
            ...
            if (_bitmask)
            {
                ...
                int nbyte = data->getZ(0).length() / 2;
        ...
        {
            int nbyteX = data->getX(0).length() / 2;
            ...
            {
                int nbyteZ = data->getZ(0).length() / 2;
        ...
        {
            int nbyteX = data->getX(0).length() / 2;
            int nbyteY = data->getY(0).length() / 2;
